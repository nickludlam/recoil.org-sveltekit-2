<script lang='ts'>
  ...
  import { faFlag, faEnvelope, faRss, faHome, faDatabase } from '@fortawesome/free-solid-svg-icons'
  ...
  import HexGrid from '$lib/components/hex-grid.svelte';
...
<div class="w-full mx-auto justify-center p-10" style="z-index:2">
  <h1 class="text-2xl md:text-4xl lg:text-6xl xl:text-8xl font-black text-gray-600">
    recoil.org/<span style="color: {secondaryColor}"><Scrambler phrases={phrases}/></span>
  </h1>
  <p>Purveyors of fine internet services since 1998</p>

  <div class="w-full">
    <h1 class="mt-10 mb-4 text-2xl font-black text-gray-600">
      SERVICES
    </h1>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {#each services as service}
      <a href={service.url}>    
        <div class="py-5 flex items-start justify-between shadow-md bg-clip-border rounded-xl bg-white hover:bg-slate-100">
            <div class="flex gap-5 pl-6">
              <div class="w-16 relative">
                {#if service.icon}
                  <HexIcon icon={service.icon} iconPadding={service.iconPadding} iconColor={iconColor} secondaryColor={secondaryColor} href="/" />
                {:else}
                  <HexIcon icon={service.icon} iconPadding={service.iconPadding} iconColor={iconColor} secondaryColor={secondaryColor} href="/">
                    {@html service.svg}
                  </HexIcon>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    {#each getRandomAvatarPair() as avatar}
      <a href={avatar.link}>
        <div class="shadow-md bg-clip-border rounded-xl w-full p-6 bg-white hover:bg-slate-300">
          <div class="flex items-center gap-4">
            <img class="w-16 h-16 rounded-xl" src={avatar.path} alt="{avatar.alt}">
            <div>
